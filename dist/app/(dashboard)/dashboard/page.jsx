...
export default function DashboardPage() {
    ...
    return (<div className="flex-1 space-y-6 p-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        {statsCards.map((stat) => (<Card key={stat.title}>
      ...
      <div className="grid gap-6 md:grid-cols-2">
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            {complianceData.map((item) => (<div key={item.label} className="space-y-2">
      ...
      <div className="grid gap-6 md:grid-cols-2">
        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-4">
              {recentActivities.map((activity, index) => (<div key={index} className="flex items-center justify-between">
                  <div className="space-y-1">
                    <p className="text-sm font-medium">{activity.type}</p>
                    <p className="text-xs text-muted-foreground">{activity.user}</p>
                  </div>
                  <div className="text-right">
                    <Badge variant={activity.status === "completed"
                ? "default"
        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-4">
              {upcomingTasks.map((task, index) => (<div key={index} className="flex items-center justify-between">
                  <div className="space-y-1">
